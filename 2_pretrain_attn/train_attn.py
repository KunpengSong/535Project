...
def sample_data(dataloader, image_size=4, batch_size=4):
    transform = transforms.Compose([
        transforms.Resize( int(1.3 * image_size) ),
...
def train_image_text_attn(total_iter, loader, options):
    ...
    copy('train_attn.py', log_folder+'/train_attn.py')
    ...
    for i in tqdm(range(options.total_iter)):
        ...
        loss_img_total = F.mse_loss(recon_img, real_image)
        '''
        ...
        loss = loss_attn_total #+ loss_txt_total + loss_img_total
        loss.backward()
        ...
        sa_mismatch_val += sa_4_mismatch.mean() + sa_8_mismatch.mean() + sa_16_mismatch.mean()
        wa_match_val += wa_4_match.mean() + wa_8_match.mean() + wa_16_match.mean()
        ...
        if (i + 1) % 1000 == 0 or i==0:
            with torch.no_grad():  
                recon_img = net_iae.decoder(img_feat_4)          
                #vutils.save_image(recon_img.detach().add(1).mul(0.5), f'{log_folder}/sample/g_img_{str(i + 1).zfill(6)}.jpg')
                g_texts = net_tae.generate(text_latent)
                f = open(f'{log_folder}/sample/g_real_txt_{str(i + 1).zfill(6)}.txt', 'w')
                for cap in g_texts:
        ...
        if (i+1) % 10000 == 0 or i==0:
            ...
            #torch.save(opt_sa4.state_dict(), f'{log_folder}/checkpoint/it_attn_{str(i + 1).zfill(6)}_opt.pth')
            torch.save({'sa4':opt_sa4.state_dict(), \
                        'sa8':opt_sa8.state_dict(), \
                        'sa16':opt_sa16.state_dict(), \
                        'wa4':opt_wa4.state_dict(), \
                        'wa8':opt_wa8.state_dict(), \
                        'wa16':opt_wa16.state_dict(), \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trial_name', default='trial_it_attn', type=str, help='name of the trial')
    ...
    tae_path = './trial_t_ae/checkpoint/it_ae_140000_model.pth'
    net_tae.load_state_dict(torch.load(tae_path)['t'])
    net_tae.eval()
    for p in net_tae.parameters():
    ...
    iae_path = './trial_i_ae/checkpoint/it_ae_080000_model.pth'
    net_iae.load_state_dict(torch.load(iae_path)['i'])
    net_iae.eval()
    for p in net_iae.parameters():
        p.requries_grad = False
    #opt_iae = optim.Adam(net_iae.parameters(), lr=args.lr, betas=(0.5, 0.999))
    ...
    opt_sa4 = optim.Adam( sa_4.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    opt_sa8 = optim.Adam( sa_8.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    opt_sa16 = optim.Adam( sa_16.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    opt_wa4 = optim.Adam( wa_4.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    opt_wa8 = optim.Adam( wa_8.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    opt_wa16 = optim.Adam( wa_16.parameters(), lr=args.lr, betas=(0.5, 0.999) )
    ...
    if args.checkpoint is not None:
        ...
        opt_sa4.load_state_dict(checkpoint['sa4'])

    print(args.path)
    loader = image_cap_loader(args.path)

    total_iter = args.total_iter
    train_image_text_attn(total_iter, loader, args)