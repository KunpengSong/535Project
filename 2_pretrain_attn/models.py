...
class ResBlock_with_text(nn.Module):
    def __init__(self, channel, text_channel):
        super().__init__()
        
        self.conv = nn.Sequential(  spectral_norm(nn.Conv2d(channel+text_channel, channel+text_channel, kernel_size=3, stride=1, padding=1, bias=False)),
                                    nn.BatchNorm2d(channel+text_channel), 
                                    nn.ReLU(),
                                    spectral_norm(nn.Conv2d(channel+text_channel, channel, kernel_size=3, stride=1, padding=1, bias=False)),
                                    nn.BatchNorm2d(channel), 
                                    nn.ReLU() )
    def forward(self, feat, text_feat):
        feat_org = F.interpolate(feat, scale_factor=2, mode='bilinear', align_corners=True)
        feat_cat = torch.cat([feat, text_feat], dim=1)
        feat_cat = F.interpolate(feat_cat, scale_factor=2, mode='bilinear', align_corners=True)
        res = self.conv(feat_cat)
        return feat_org + res
...
class WordAttention(nn.Module):
    ...
    def rf(self, img_feat, word_seq):
        ...
        proj_value_from_text = proj_value_from_text.view(b, -1)  #.view(b*self.word_length*c, -1)
