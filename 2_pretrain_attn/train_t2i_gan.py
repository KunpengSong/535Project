...
def train_image_gan_with_text(net_ig, net_id, opt_ig, opt_id, total_iter, loader, options):
    ...
    copy('train_t2i_gan.py', log_folder+'/train_t2i_gan.py')
    ...
    for i in tqdm(range(options.start_iter, options.total_iter)):
        ...
        g_image = net_ig(noise, real_text_latent, real_embs, step)
        g_pred = net_id(g_image, step)
        g_pred_s, g_pred_w = net_idt(g_image, real_text_latent, real_embs, step)

        loss_g_latent = -g_pred.mean() - g_pred_s.mean() - g_pred_w.mean()
        ...
        image_g_val += g_pred.mean().item()
        image_gt_val += g_pred_s.mean().item() + g_pred_w.mean().item()
        ...
        fake_predict = net_id(g_image.detach(), step)
        ...
        fake_predict_s, fake_predict_w = net_idt(g_image.detach(), real_text_latent, real_embs, step)
        mismatch_predict_s, mismatch_predict_w = net_idt(real_image, real_text_latent[perm], real_embs[perm], step)

        loss_disc = F.relu(1-real_predict_s).mean() + F.relu(1-real_predict_w).mean() + \
                    F.relu(1+fake_predict_s).mean() + F.relu(1+fake_predict_w).mean() + \
        ...
        if (i + 1) % 2000 == 0 or i==0:
            with torch.no_grad():            
                vutils.save_image(g_image.detach().add(1).mul(0.5), f'{log_folder}/sample/g_img_{str(i + 1).zfill(6)}.jpg')
        ...
        if (i+1) % 5000 == 0 or i==0:
            torch.save({'ig':net_ig.state_dict(), 'id':net_id.state_dict(), 'idt':net_idt.state_dict()}, f'{log_folder}/checkpoint/image_to_text_memory_{str(i + 1).zfill(6)}_model.pth')
            torch.save({'ig':opt_ig.state_dict(), 'id':opt_id.state_dict(), 'idt':opt_idt.state_dict()}, f'{log_folder}/checkpoint/image_to_text_memory_{str(i + 1).zfill(6)}_opt.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trial_name', default='trial_t2i_gan_with_pre-trained_sw', type=str, help='name of the trial')
    ...
    train_image_gan_with_text(net_ig, net_id, opt_ig, opt_id, total_iter, loader, args)